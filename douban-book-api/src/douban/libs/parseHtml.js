...
function getInfo($) {
  const info = {};
  const infoEle = $('#info');
  if (!infoEle.length) return info;
  const spanEles = [...infoEle.children('span')];
  spanEles.forEach(spanEle => {
    if (spanEle.children.length === 1 && spanEle.children[0].data) {
      const key = spanEle.children[0].data.remove(':', ' ');
      let value = '';
      if (spanEle.next && spanEle.next.next && spanEle.next.next.name === 'a') {
        value = $(spanEle.next.next).text();
      } else if (spanEle.next && spanEle.next.data) {
        value = spanEle.next.data.slice(1);
      }
      info[key] = value;
    } else {
      const children = spanEle.children.filter(ele => ele.name === 'span' || ele.name === 'a');
      if (children.length > 0 && children[0].children && children[0].children[0] && children[0].children[0].data) {
        info[children[0].children[0].data.remove(' ')] = children.slice(1).map(aEle => aEle.children[0]?.data || '').filter(Boolean);
      }
    }
  });
...
function getCoverUrl($) {
  const coverLink = $('div#mainpic>a.nbg')[0];
  return coverLink ? coverLink.attribs.href : '';
}
...
function parseHTML(html, id) {
  ...
  if (!id) {
    const idMatch = /douban\.com\/book\/subject\/(\d+)\//.exec(html);
    id = idMatch ? idMatch[1] : '';
  }
  
  let comments = [];
  try { comments = getComments($); } catch (e) { console.error('getComments error:', e.message); }
  
  let reviews = [];
  try { reviews = getReviews($); } catch (e) { console.error('getReviews error:', e.message); }
  
  let rating = { count: 0, info: '', value: 0, five_star_per: 0, four_star_per: 0, three_star_per: 0, two_star_per: 0, one_star_per: 0 };
  try { rating = getRating($); } catch (e) { console.error('getRating error:', e.message); }
